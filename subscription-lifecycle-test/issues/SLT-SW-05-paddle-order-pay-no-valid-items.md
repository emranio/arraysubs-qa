...
- Product `12611` still had empty `_arraysubs_gateway_paddle_product_id`, `_arraysubs_gateway_paddle_price_id`, and `_arraysubs_gateway_paddle_synced_at` metadata. This matches the catalogue-sync failure tracked in `issues/SLT-SETUP-05-paddle-product-sync-metas-not-created.md` and is a likely contributing condition; it is not proof of root cause.
