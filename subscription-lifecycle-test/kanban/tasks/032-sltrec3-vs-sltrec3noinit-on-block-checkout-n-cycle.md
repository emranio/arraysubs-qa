---
id: 32
title: SLT2REC3 vs SLT2REC3NOINIT on block checkout - N-cycle counting and the exact renewal where the discount stops
status: todo
priority: critical
...
- Both checkouts **18:00-19:00 site (UTC+6) on 2026-08-25**, same hour so the ladders align. Satisfies C02; keeps every invoice leg (12:00-19:00 site) clear of SLT-SYN-04's D3 09:00-11:00 bracket.
- Coupon expiry must be `2026-09-05` (date-only expiry is site-local midnight, so this is the first value that covers all of D12).
...
| Item | Value |
|---|---|
| Product | SLT2 Daily Core $10.00 day/1 (both) |
| Accounts | slt2-cpncyc = `SLT2REC3`; slt2-cpncyc2 = `SLT2REC3NOINIT` |
| Card | 4242 4242 4242 4242 |

| Leg | Sub A (remaining) | Sub B (remaining) |
|---|---|---|
| checkout 08-25 | $7.50, 3->**2** | $7.50, stays **3** |
| renewal #1 08-26 | $7.50 -> 1 | $7.50 -> 2 |
| renewal #2 08-27 | $7.50 -> 0 | $7.50 -> 1 |
| renewal #3 08-28 | **$10.00 full** | $7.50 -> 0 |
| renewal #4 08-29 | $10.00 | **$10.00 full** |

## Steps
1. `USER_PRE=$(mailpit-agent latest-id)` and `SUBCOUNT_BEFORE=<exact current SLT2 subscription count>`. In `agent-browser --session admin-SLT-CPN-03`, create `slt2-cpncyc` and `slt2-cpncyc2` (Customer, **Send User Notification** unticked, `SltQa!2026#Pass`, billing per SLT-SETUP-03); record both numeric user IDs.
2. Open both coupons; screenshot each ArraySubs group; confirm **Count initial checkout** TICKED on SLT2REC3 and UNTICKED on SLT2REC3NOINIT, both cycles = 3, expiry 2026-09-05. Record coupon IDs.
...
7. For each subscription and cycle, store a distinct registry baseline `REN_PRE_<ALIAS>_<N>=$(mailpit-agent latest-id)` no earlier than five minutes before that subscription's exact recorded charge gate. **Mornings 08-27, 08-28, 08-29 and 08-30 after 09:00 site**: reopen `admin-SLT-CPN-03`; for each exact new renewal order resolve its subscription relationship, screenshot the item table with a unique alias/cycle filename, run `wp eval '$o=wc_get_order(<REN>);foreach($o->get_fees() as $f){echo $f->get_name()."|".$f->get_total();}echo "|".$o->get_total();' --allow-root`, re-read `_coupon_remaining_cycles`, copy the newest note, run `mailpit-agent wait-new "<that exact recorded baseline ID>" 900 "Payment received for subscription #<exact numeric subscription ID>"`, reconcile every message newer than that exact baseline, and append every renewal/action/baseline/mail ID plus the next cycle's `charge−5m` deadline to `slt2-catalog-registry` and that day's watch report. Close the admin session after each morning. After the D7 morning proves `SUB_CPN_NOINIT` renewal #4 at full price, independently review the complete D2-D7 record and move this card through review to done.
...
5. **`SUB_CPN_INIT` renewal #3, charged the evening of 2026-08-28, totals $10.00 with ZERO fee items** (`applyRecurringCoupons()` returns on `original>0 && remaining<=0`). **Drop-off proved by the watch run on the morning of 2026-08-29 = watch day D6.**
6. `SUB_CPN_NOINIT` renewals #1/#2/#3 each total **$7.50** with fee `Recurring Discount: sltrec3noinit` = -2.50; remaining 2, 1, 0.
7. **`SUB_CPN_NOINIT` renewal #4, charged the evening of 2026-08-29, totals $10.00 with ZERO fee items. Drop-off proved by the watch run on the morning of 2026-08-30 = watch day D7.**
...
- [ ] `SUB_CPN_INIT`: exactly 2 discounted renewals, then $10.00 on 2026-08-28
- [ ] `SUB_CPN_NOINIT`: exactly 3 discounted renewals, then $10.00 on 2026-08-29
...
## Isolation / teardown
- Watch: D4 both still discounted; **D6 (08-29) `SUB_CPN_INIT` at $10.00**; **D7 (08-30) `SUB_CPN_NOINIT` at $10.00**; D8-D9 both $10.00.
