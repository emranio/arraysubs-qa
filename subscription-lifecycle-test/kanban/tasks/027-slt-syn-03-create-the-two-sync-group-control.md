---
id: 27
title: 'SLT-SYN-03 Create the sync-group control product: SLT2 Sync Global Daily'
status: todo
priority: critical
...
> **SLT-SYN-03** · group `sync` · scheduled **D01** (2026-08-24)

> Read `README.md` (environment + isolation contract), `calendar.md` (this day's exact
> ordering — it is binding, not advisory) and `plan-audit.md` before starting.

## Objective
Create the one NEW product this dimension owns. The catalog has no non-flex product with a cycle of 3+ days, so `SLT2 Sync Global Daily` is required to isolate what plain global `sync_to_billing_cycle` does on its own. The former `SLT2 Sync Excl Probe` is deliberately not created: `SLT-PROD-05` already owns the identical Different Renewal Price versus Flexible Renewal Sync exclusivity proof.
...
## Preconditions
- SLT-SETUP-01 (conventions: `SLT <Name>` title, `slt2-<name>` slug, Virtual, stock management off) and SLT-SETUP-02 (global sync OFF baseline) complete.
- SLT-PROD-14 complete — `SLT2 Sync Global Daily` deliberately mirrors its day/3 cycle so it is an exact non-flex control for `SLT2 Flex Daily Two Seg` and `SLT2 Flex Daily Next Cycle`.
- `SLT2 Sync Global Daily` is declared NEW by this task. No other group may buy it.
...
| Item | Value |
|---|---|
| Product | SLT2 Sync Global Daily / slug `slt2-sync-global-daily` — Simple, Virtual, subscription, day/3, length 0, trial 0, no signup fee, NO flex sync |
...
3. **Product title**: `SLT2 Sync Global Daily`. **Description**: `SLT2 window product. Non-flex day/3 control for global renewal sync. Owned by SLT-SYN. Delete on 2026-09-05.`
...
9. Set the URL slug to `slt2-sync-global-daily`. **Publish**. Reload the edit screen and re-verify every field.
10. Verify metas from WP root `cd /home/server-manager/www/arrayhash/mirror-help.arrayhash.com/public`:
    `wp post meta list <SLT2 Sync Global Daily ID> --keys=_is_subscription,_subscription_period,_subscription_interval,_subscription_length,_trial_length,_signup_fee,_enable_renewal_price,_arraysubs_flex_sync_enabled,_regular_price --allow-root`.
11. Confirm the segment-plan resolver agrees: `wp eval 'use ArraySubsPro\Features\FlexibleRenewalSync\Services\SegmentPlan; var_dump(SegmentPlan::getNominalCycleDays("day",3)); var_dump(SegmentPlan::getConfig(<SLT2 Sync Global Daily ID>));' --allow-root`.
...
13. As `--session guest-SLT-SYN-03`, open `https://mirror-help.arrayhash.com/product/slt2-sync-global-daily/?slt2-cache-bust=<timestamp>`, confirm it renders a recurring "every 3 days" schedule summary, capture `SLT-SYN-03-02-global-daily-frontend.png`, and do NOT add anything to the cart.
14. Append the product ID and verified Shop Access exclusion to the `slt2-catalog-registry` page as `sync-group non-flex control`. In the same registry entry, link the `SLT-PROD-05` screenshot IDs as the authoritative exclusivity evidence and state that no `SLT2 Sync Excl Probe` product was created.
...
## Expected results
1. `SLT2 Sync Global Daily` published: type `simple`, virtual, slug exactly `slt2-sync-global-daily`, `_is_subscription=yes`, `_subscription_period=day`, `_subscription_interval=3`, `_subscription_length=0`, `_trial_length=0`, `_signup_fee` absent or `0`, `_enable_renewal_price` absent, `_regular_price=18.00`, and `_arraysubs_flex_sync_enabled` ABSENT.
...
6. The registry points to `SLT-PROD-05` for the already-covered exclusivity proof; no product with slug `slt2-sync-excl-probe` exists.
...
## Pass criteria
- [ ] SLT2 Sync Global Daily published day/3 at $18.00 with no flex meta and no renewal-price meta
...
- [ ] No `slt2-sync-excl-probe` product created
- [ ] Zero mail; nothing added to cart; no existing product touched

## Isolation / teardown
- State handoff: `SLT2 Sync Global Daily` is bought exactly once by `SLT-SYN-04` as `slt2-flex`, while global sync is temporarily ON. No other task may purchase it.
- Cross-purpose note recorded deliberately: this product is bought by `slt2-flex` even though it is not a flexible-sync product, because it exists solely as this dimension's control and `slt2-core` is reserved for checkout workhorses.
