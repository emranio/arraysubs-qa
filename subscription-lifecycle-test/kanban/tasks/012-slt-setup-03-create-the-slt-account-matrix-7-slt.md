...
- The original zero-mail expectation was a plan defect: unchecking Send User Notification suppressed customer mail, while WordPress emitted one admin-only New User Registration message per created user. The task contract was corrected directly; the captured set is exactly nine admin messages and zero customer messages. See `issues/SLT-SETUP-03-admin-new-user-mail-despite-notification-checkbox-off.md`.
