...
D12 initial watch verdict: FAIL — `issues/done-SLT-SYN-04-successful-renewal-leaves-stale-pending-order-pointer.md`.

[[2026-08-14]] Fri 17:44
RESOLVED. Persisted-order regression coverage now blocks stale pending-result resurrection and forged paid results, exact paid cleanup preserves newer invoices, and the pro gateway normalizer reloads the provider-completed order. The exact stale pointer `12564 -> 20230` was compare-and-deleted under the subscription mutation lock after relationship/payment preconditions passed. Four unrelated non-paid pointers remain untouched; site-wide paid-pointer count is zero. Browser verification reconfirmed the Active subscription and completed USD 18.00 renewal. Fix commits: arraysubs `69e0fe8`, arraysubspro `09ad013`; evidence and full resolution are in the prefixed issue above.
