---
id: 95
title: Crossgrade SLT2 Plan Pro to the equal-priced SLT2 Plan Peer and prove the date does not shift
status: todo
priority: high
...
> **SLT-SW-03** · group `switching` · scheduled **D06** (2026-08-29)

> Read `README.md` (environment + isolation contract), `calendar.md` (this day's exact
> ordering — it is binding, not advisory) and `plan-audit.md` before starting.

## Objective
Crossgrade `slt2-switch`'s **SLT2 Plan Pro** subscription (`SUB_PRO`, $15.00 day/1) to the equal-priced **SLT2 Plan Peer** ($15.00 day/1) and prove the money is a wash — classification `crossgrade` via the ±5 % daily-rate tolerance, credit exactly equal to charge, **Amount due $0.00**, no proration order, no store credit — and that the next payment date does not silently move.
...
- `slt2-switch` owns the **active** `SUB_PRO` bought by `SLT-SW-00` — not `SUB_BASIC`, which SLT-SW-01 upgraded.
...
| Item | Value |
|---|---|
| Subscription | `SUB_PRO` (slt2-switch / SLT2 Plan Pro) |
...
4. Log in as `slt2-switch` / `SltQa!2026#Pass`; open the portal page; **Change Plan**.
5. The modal must expose a second tab **Others** — SLT2 Plan Peer is a crossgrade and is rendered there with the badge **Change**, not under **Upgrade/Downgrade**. Screenshot both tabs.
6. **Select** SLT2 Plan Peer; record the preview rows; **Amount due** must read `$0.00` and no "You will receive a credit" line may appear.
7. Confirm → **Change Plan**. Expect the success toast and a reload; no redirect to order-pay.
8. Re-dump the meta; re-read both pending legs for `[SUB_PRO]`; check exact before/after HPOS order counts for `slt2-switch` — no order may have been created. Publish the two replacement action IDs/GMTs, exact charge gate, and `charge−300s` deadline to the registry and daily report.
...
4. `_product_id`=Peer, `_recurring_amount`=`15.00`, `_billing_period=day`, `_billing_interval=1`, title `SLT2 Plan Peer - Subscription #<SUB_PRO>`.
5. `_next_payment_date` is **byte-identical** to step 1 — Branch A returns the current date unchanged.
6. Both renewal legs are re-created with new action IDs but identical scheduled GMT values. A shifted GMT creates/updates the mandatory `qa/issues/` kanban card with both queue screenshots.
...
- [ ] Product/title/recurring amount updated to SLT2 Plan Peer at $15.00
...
## Isolation / teardown
- Hands `SUB_PRO` to SLT-SW-02 sitting on **SLT2 Plan Peer** (Peer's downgrade list = [Basic]). Do not switch it again.
