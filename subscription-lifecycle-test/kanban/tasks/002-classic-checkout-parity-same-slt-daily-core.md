...
- The raw surface difference—classic subscription omitted `_shipping_address`—is documented only in `issues/SLT-CHK-02-classic-checkout-omits-shipping-address-meta.md`. Gateway duplicate-row behavior is updated only in `issues/SLT-OBS-01-duplicate-gateway-postmeta-rows.md`; `11991` had no duplicate keys.
