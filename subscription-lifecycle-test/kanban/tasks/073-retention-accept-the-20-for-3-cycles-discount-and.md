...
> **SLT-SW-09** · group `switching` · scheduled **D04** (2026-08-27)
...
- Gateway: Stripe test
- Checkout: block
- Account: new registered — **this task creates `slt2-retain` + `slt2-retain2`**
- Plugins: free-only
...
| Item | Value |
|---|---|
| A | slt2-retain buys **SLT2 Plan Basic** $5.00 day/1; discounted renewal **$4.00** (line −$1.00) |
| B | slt2-retain2 buys **SLT2 Plan Peer** $15.00 day/1; downgrade target **SLT2 Plan Basic** $5.00. Card 4242 4242 4242 4242 |
| Watch | A renews D5/D6/D7 at $4.00 and **D8 (08-31) at $5.00** — reported on watch D6-**D9** |
...
4. `SUB_RET_DOWN` after step 7: `_retention_offer_type=downgrade`, `_waiting_cancellation` deleted, still active, `_recurring_amount` **15.00**, pending switch to Basic ("Plan switch to SLT2 Plan Basic scheduled for …").
...
| 2 | retention_discount_accepted | step 4 | slt2-retain | `Your retention discount for SLT2 Plan Basic is active` | `mailpit-agent wait-new "$RET_PRE" 180` |
...
| 5 | payment_successful | each watched paid renewal | slt2-retain / slt2-retain2 | `Payment received for subscription #<exact id>` | complete owner delta after the matching `RET_DISC_Rn_PRE` / `RET_DOWN_Rn_PRE`; save/show exact id |
...
## Isolation / teardown
- Both subscriptions stay active through watch day D9 (2026-09-01); SLT-SETUP-99A must not cancel them before their evidence is complete.
