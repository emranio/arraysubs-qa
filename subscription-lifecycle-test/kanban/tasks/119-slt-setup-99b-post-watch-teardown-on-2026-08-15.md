...
## Steps
1. Record `M0=$(/usr/local/bin/mailpit-agent latest-id 2>/dev/null || true)`, then cancel the tail cohort that `SLT-SETUP-99A` deliberately kept alive. Build that list from 99A's published keep-alive registry, not from memory, and require it to include every live long-tail fixture still handed to D13: all three week-segment subscriptions (`SUB_W1`, `SUB_W`, `SUB_W3`), `SUB_2SEG` if 99A kept it alive per task 106, Sync Global Daily, the two `SLT-SYN-12` probes, `SLT-SYN-14`'s quantity subscription, Box Daily, both primary lifetime controls, both flex-month subscriptions (`SUB_M`, `SUB_S3`), and all three `SLT-SYN-11` probes (including Excl Lifetime Probe). Include the two `SLT-SYN-13` variation subscriptions only if the live registry proves they exist despite task 46's appended 2026-08-05 `UNVERIFIED` closeout note. Inspect every message after `M0` (paginate the localhost Mailpit API if needed), reconcile each cancellation message to one exact tail subscription, and prove no subscription outside the published tail appears. Then record `M1=$(/usr/local/bin/mailpit-agent latest-id 2>/dev/null || true)` as the deletion-step baseline.
...
Current evidence correction before D13 teardown: rebuild the real tail only from 118's published keep-alive cohort plus the live registry. From the Thursday, August 6, 2026 board state, task 106 still requires `SUB_2SEG` to survive past D10 and this card's step-1 wording was corrected the same evening to include it when 118 keeps it alive. Task 46 still says the two `SLT-SYN-13` variation subscriptions never existed and they should stay conditional-only unless a later valid execution proves otherwise. Separately, do not add any ladder-switch or auto-downgrade artifacts to the tail unless a later valid execution recreates task 72's missing `SUB_BASIC` / `SUB_PRO` chain and 118 publishes those live fixtures explicitly.
