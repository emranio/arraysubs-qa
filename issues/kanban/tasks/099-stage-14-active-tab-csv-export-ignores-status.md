...
Fixed and verified. Code: DataList passes current statusFilter to custom buttons; SubscriptionsList adds status to export URL when current tab is not All. Check: npm run build passed. Browser: agent-browser confirmed active filter URL + Export CSV button. agent-browser clicked Active(181), exported CSV, verified 181 data rows, every Status=Active, BOM present, filename subscriptions-export-2026-05-24-133732.csv. Artifacts: qa/artifacts/issue-99-active-tab-before-export.png and qa/artifacts/issue-99-active-export.csv.
