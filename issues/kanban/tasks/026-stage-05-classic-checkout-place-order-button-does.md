...
Result: Added core checkout guards to disable Woo Stripe expanded Optimized Checkout and Stripe express checkout buttons on ArraySubs subscription classic checkout. Retest: normal agent-browser click still fails before PHP with /?wc-ajax=checkout statusText 'TypeError: Illegal invocation'. Root cause isolated to agent-browser's injected XMLHttpRequest wrapper calling nativeOpen(method, url, ...rest) without binding this. After a temporary in-page XHR repair, the same agent-browser session completed BACS checkout: order #800, total 9.99, payment Direct bank transfer, related subscription #801 pending. Cleaned diagnostic artifacts #793/#794 and verification artifacts #800/#801. Restored woocommerce_checkout_page_id to #8. Blocked because default agent-browser cannot verify WooCommerce jQuery checkout AJAX until tool XHR wrapper is fixed; adding a product-side agent-browser-specific shim would be wrong.
