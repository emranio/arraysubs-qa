...
Fixed Store Credit route scoping: /store-credit/:userId now treats route param as source of truth, clears stale customer data on route changes, reloads the scoped customer every time userId changes, guards stale responses, and Clear resets URL to /store-credit. Verified via agent-browser: Manage Members #32 -> Manage Store Credit opens #/store-credit/32 scoped to Customer One with 30.00 balance and history; #/store-credit/33 switches to Customer 2 with 0.00/no history; #/store-credit/32 switches back to Customer One with history. agent-browser screenshot qa/artifacts/issue-110/store-credit-customer-one-scoped.png. npm run build passed.
