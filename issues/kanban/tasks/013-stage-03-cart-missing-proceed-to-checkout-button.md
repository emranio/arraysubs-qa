...
Fixed: added a server-rendered Proceed to checkout fallback inside the Woo Cart Block proceed-to-checkout block for trial-only subscription carts where the Blocks frontend omits the button. Verified fresh Alumnium flow: add Trial Weekly to cart, cart line and trial metadata load, visible Proceed to checkout link points to checkout, clicking it opens Checkout page.
